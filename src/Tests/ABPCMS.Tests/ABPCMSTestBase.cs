...
namespace ABPCMS.Tests
{
    public abstract class ABPCMSTestBase : AbpIntegratedTestBase<ABPCMSTestModule>
    {
        ...
        protected void UsingDbContext(Action<ABPCMSDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ABPCMSDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ABPCMSDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ABPCMSDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ABPCMSDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABPCMSDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ABPCMSDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABPCMSDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ABPCMSDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ABPCMSDbContext, Task<T>> func)
        {
            T result;
